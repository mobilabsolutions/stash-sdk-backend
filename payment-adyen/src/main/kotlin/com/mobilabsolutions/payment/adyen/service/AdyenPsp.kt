...
class AdyenPsp(
    ...
    override fun registerAlias(pspRegisterAliasRequestModel: PspRegisterAliasRequestModel, pspTestMode: Boolean?): PspRegisterAliasResponseModel? {
        ...
        if (pspRegisterAliasRequestModel.aliasExtra?.paymentMethod == PaymentMethod.CC.toString()) {
            val adyenMode = getAdyenMode(pspTestMode)
    ...
    override fun preauthorize(pspPaymentRequestModel: PspPaymentRequestModel, pspTestMode: Boolean?): PspPaymentResponseModel {
        ...
        val response = when {
            pspPaymentRequestModel.extra?.paymentMethod == PaymentMethod.CC.toString() -> makeCreditCardPayment(pspPaymentRequestModel, adyenMode, false)
    ...
    override fun authorize(pspPaymentRequestModel: PspPaymentRequestModel, pspTestMode: Boolean?): PspPaymentResponseModel {
        ...
        val response = when {
            pspPaymentRequestModel.extra?.paymentMethod == PaymentMethod.CC.toString() -> makeCreditCardPayment(pspPaymentRequestModel, adyenMode)
            pspPaymentRequestModel.extra?.paymentMethod == PaymentMethod.SEPA.toString() -> makeSepaPayment(pspPaymentRequestModel, adyenMode)
            else -> throw ApiError.ofErrorCode(ApiErrorCode.PSP_MODULE_ERROR, "Unexpected payment method").asException()
        }
        if (response.resultCode == AdyenResultCode.ERROR.result ||
            response.resultCode == AdyenResultCode.REFUSED.result ||
    ...
    override fun refund(pspRefundRequestModel: PspRefundRequestModel, pspTestMode: Boolean?): PspPaymentResponseModel {
        ...
        val response = when {
            pspRefundRequestModel.paymentMethod == PaymentMethod.CC.toString() -> makeCreditCardRefund(pspRefundRequestModel, adyenMode)
            pspRefundRequestModel.paymentMethod == PaymentMethod.SEPA.toString() -> makeSepaRefund(pspRefundRequestModel, adyenMode)
            else -> throw ApiError.ofErrorCode(ApiErrorCode.PSP_MODULE_ERROR, "Unexpected payment method").asException()
        }
        if (response.resultCode == AdyenResultCode.ERROR.result ||
            response.resultCode == AdyenResultCode.REFUSED.result ||
