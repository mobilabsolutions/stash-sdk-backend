...
enum class ApiErrorCode(val code: String, val message: String, val httpStatus: HttpStatus) {
    ...
    MERCHANT_USER_NOT_FOUND("1005", "Merchant user with given email cannot be found.", HttpStatus.BAD_REQUEST),
    TOKEN_NOT_FOUND("1006", "Password reset token does not exist.", HttpStatus.BAD_REQUEST),
    TOKEN_EXPIRED("1007", "Password reset token expired.", HttpStatus.BAD_REQUEST),
